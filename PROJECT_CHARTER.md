...
`ade run --plan plan.json` e `ade run --unattended` sob precondições duras; v0.5 entregue (pesquisa controlada, telemetria, drenagem DRAINING -> STOPPED, pausa, retomada e takeover PTY); v1 ativa com dogfood Vitest, calibração por telemetria e fechamento documental; JS ESM com JSDoc (ADR 0023) em migração para TypeScript estrito sem build no backend e build do painel só em `packages/web` (ADR 0030), ADR 0028 e ADR 0029.
