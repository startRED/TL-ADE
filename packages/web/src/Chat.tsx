...
import { Badge, Button, Flex, Heading, Select, Text, TextArea, TextField } from '@radix-ui/themes'
    ...
    <aside className="rail" aria-label="Conversa">
      ...
      <form className="composer" onSubmit={submit}>
        ...
        <Flex gap="2" align="center" wrap="wrap">
          <Select.Root size="1" value={family} onValueChange={(v) => setFamily(v as typeof family)}>
            <Select.Trigger aria-label="Empresa" variant="soft" color="gray" />
            <Select.Content>{FAMILIES.map((f) => <Select.Item key={f} value={f}>{f}</Select.Item>)}</Select.Content>
          </Select.Root>
          ...
          <Select.Root size="1" value={effort} onValueChange={(v) => setEffort(v as typeof effort)}>
            <Select.Trigger aria-label="Esforço" variant="soft" color="gray" />
            <Select.Content>{EFFORTS.map((f) => <Select.Item key={f} value={f}>{f}</Select.Item>)}</Select.Content>
          </Select.Root>
