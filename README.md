...
- `ade models [plan <familia> <plano> | block <modelo> | unblock <modelo> | effort <papel> <esforco> | quota <familia> <percentual> --resets <iso>]`: mostra planos, cota considerada e a fila de modelos de cada papel com o porquê; os subcomandos definem o plano, bloqueiam modelos, fixam o esforço de um papel e informam a cota à mão (ADR 0033).
- `ade dogfood --example <pasta> --out <arquivo.md> [--double]`: Ponto de virada ponta a ponta. Copia o exemplo para uma pasta temporária com git e sem remoto, leva o `PEDIDO.md` pelo painel real e executa com `ade run --plan`. As filas vêm dos planos da página Modelos, e o comando sai 4 sem duas empresas. Grava o registro gerado do journal; `--double` troca só a fronteira de modelo.
