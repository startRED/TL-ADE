...
| 4 | `skills` | story (≤3 ids) | 2.500/skill, 7.500 total | `ade show skill:<id>` |
| 5 | `retrieved_context` | story | 6.000 tok | `ade show art:<ref>` |
| 6 | `contract` | story | 2.000 tok | `ade show contract:<story>` |
| 7 | `round` | rodada | 6.000 tok (findings 3.000 / gate_failures 2.000 / checkpoint 1.000) | `ade show art:<ref>` |
| 8 | `task` | papel | 300 tok | não trunca |

Soma dos tetos por seção: ~24,3k tokens, contra o teto global de 40k — a folga é deliberada: o corte
...
| 8 `task` | schema de `unit-result` + caminho do `result_file` | schema de `review-result` | idem | schema `visual-eval` inline | schema da classificação | schema `research-finding` inline |
...
Teto global: **40.000 tokens** por pack [hipótese] — sem base empírica própria
(`landscape-context-observability.md §7.5`). Consequências operacionais:

1. A validação do plano (`ade validate`) estima o pack de Maker de cada story e **recusa a story**
   acima do teto com `pack_budget_exceeded`, devolvendo-a ao Intent Compiler para divisão
   (`architecture.md §5`, passo 8). Recusa, nunca truncagem silenciosa.
...
| :--- | :--- | :--- |
| `claude` | `--safe-mode` + `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`; **nunca `--bare`** | `--bare` quebra a autenticação por assinatura (digest #9); a auto memory vem ligada por padrão, escreve por repositório e é local à máquina — quebra o determinismo do pack e a paridade de testes (`landscape-context-observability.md §3.4`) |
| `codex` | `--ignore-user-config` em toda chamada | piso de ~19,4k tokens de entrada vindo de instruções + `AGENTS.md` + catálogo de skills (digest #27) |
| `agy` | somente-leitura até o canário de isolamento passar | escreveu em `~/.gemini/antigravity-cli/scratch/` em vez do `--add-dir` pedido, sem aviso (digest #38) |
...
<<<ADE_TOOL_OUTPUT ref="art:gates/vitest/0007" trust="untrusted" kind="test"
    exit="1" bytes_raw="412903" bytes_model="6114">>>
...
<<<END_ADE_TOOL_OUTPUT>>>
```

O engine escapa qualquer ocorrência literal dos delimitadores dentro do payload antes de cercar
(`<<<ADE_TOOL_OUTPUT` → `<<<ADE_TOOL_OUTPUT​`), senão o conteúdo fecha a própria cerca. O
...
(`landscape-context-observability.md §4.1`). Na v0.4 a projeção SQLite materializa as agregações e é
reconstruível a partir do journal — o índice é cache, nunca fonte.
...
**modelo do juiz fixado** no contrato (`landscape-evals-visual.md §5.2`): sem isso a métrica anda
quando o modelo muda, não quando a qualidade muda.
...
| Taxa de citação | `cited = true` ÷ injeções | `sources` do resultado (§5.1) |
...
| Por repositório | v0.x | `<repo>/.ade/memory/` versionado em git: `INDEX.md` (uma linha por nota, teto 200 linhas / 25 KB — os mesmos limites da auto memory do Claude Code) + notas por tópico lidas sob demanda. Conteúdo: decisões, armadilhas do repo, comandos que funcionam, evals que nunca falharam. Escrita por step explícito `memory_write` no fim da story, com o mesmo `contain` de qualquer efeito |
| Por usuário | **fora** | preferência do operador cabe em `~/.ade/config.json`. Sem evidência de ganho e com risco de vazar contexto entre projetos |

`<repo>/.ade/memory/` é a única parte de `.ade/` versionável; o resto continua fora do commit
(`architecture.md §2`). Mem0/Letta/Zep estão rejeitados em `docs/adr/0019-rejeicoes.md`: os benchmarks
...
papel a preenchê-la. Sem isso, `cited` é sempre `false` e a v1 coleta ruído.
...
fora do caminho). Proposta: o corte do compilador é **em bytes** (`max_pack_bytes`, valor a calibrar
por telemetria), e "40k tokens" vira o alvo de projeto medido por estimativa, reportado em
`pack_bytes` + estimativa, nunca o gatilho do corte. Duas unidades para a mesma regra é defeito
esperando o primeiro pack multibyte.
...
a seção 5 do Checker respeita o teto de 6.000 tokens como qualquer outra, com diff por arquivo
ordenado por relevância de escopo e ponteiro `ade show diff:<story>#<arquivo>` para o resto; story cujo
diff não cabe é sinal de story mal dimensionada, que é a mesma regra da §1.3.
...
depender do valor medido, não da flag.
...
(Context Rot), não por custo.

---

## 9. Perguntas em aberto

1. Qual `max_pack_bytes` real? O valor do runtime (60.000) foi calibrado para packs sem skills e sem
   `graft_context`; a ADE acrescenta três seções. Calibrar por p90 da telemetria após as primeiras 20
   stories.
