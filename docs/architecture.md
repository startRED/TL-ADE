...
do repo (≤1,5k) → skills por id (≤25k) → contexto recuperado (≤6k) → contrato → rodada (achados,
...
- E14 Skills: ≤10k tokens por skill e soma ≤25k (elevado de 5k/7,5k por E70; não 2,5k fixo); o filtro duro não elimina por tamanho antes do BM25. Braço de controle "BM25@3 puro" medido antes de manter o seletor barato. E: skill-fabric D1; adr-1.
...
- E70 ADO Teto por skill sobe de 5k para **10k tokens** e a soma do bloco de skills de 7,5k para **25k** (≤3 skills; o bloco de skills passa a ser a maior seção do pack, dentro de `max_pack_bytes`). Justificativa: skills só entram na chamada do Maker, o bloco vai no prefixo estável do pack e é servido pelo cache de prompt (`cache_read` na telemetria), então o custo marginal por chamada é ~10 %. Continua [hipótese]: se `cache_read` ficar abaixo de 80 % do bloco ou a obediência cair (achados do Checker por regra de skill ignorada), o teto volta a 5k. Permite `taste-skill` (recorte) e `img2threejs` sem poda agressiva.
