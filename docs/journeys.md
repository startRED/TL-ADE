...
| Teto do bloco de skills no pack | ≤5k tok por skill, soma ≤7,5k | architecture.md §11 E14 |
...
| **Skills** | ≤3, após filtro duro (domínio `frontend`, linguagem `ts`) → BM25 top-8 → seletor barato: `frontend-design` (local, vence por nome), `improve-ui` (`ibelick/ui-skills`), `accessibility`. Soma ≤7,5k tokens, cada skill ≤5k (§11 E14); o filtro duro não elimina por tamanho antes do BM25. |
