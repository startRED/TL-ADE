...
| Teto do bloco de skills no pack | ≤10k tok por skill, soma ≤15k | architecture.md §11 E14 |
...
| **Skills** | ≤3, após filtro duro (domínio `frontend`, linguagem `ts`) → BM25 top-8 → seletor barato: `frontend-design` (local, vence por nome), `improve-ui` (`ibelick/ui-skills`), `accessibility`. Soma ≤15k tokens, cada skill ≤10k (§11 E14, §12 E70); o filtro duro não elimina por tamanho antes do BM25. |
