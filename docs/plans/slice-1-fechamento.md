...
| S1-02 | slice-1.md §7.2 | tests/parity/crash.test.ts e inventário abaixo | não comprovado | não comprovado | testes encontrados; nenhuma matriz por SO executada nesta conferência | não comprovado | faltam 12 células dedicadas e resultados Windows e Linux |
| S1-03 | slice-1.md §7.3 | tests/**/*.test.ts e inventário abaixo | não comprovado | não comprovado | 70 nomes exigidos localizados por busca de fonte; não executados por SO | não comprovado | faltam relatórios Windows e Linux por nome e prova de paralelismo por worker |
...
| RM-COBERTURA | roadmap.md §1; slice-1.md §6 | não comprovado | não comprovado | não comprovado | nenhum relatório de cobertura foi localizado | não comprovado | falta cobertura ≥85% em journal, step, lease, git, runner e contain |
