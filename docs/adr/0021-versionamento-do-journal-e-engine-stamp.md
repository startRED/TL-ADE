...
| `runtime_stamp` | `<versão do engine>:<digest da config>` | a cada build e a cada mudança de `.ade/config.json` | intenção aberta com stamp divergente para a missão = `stale_workflow_version`, para até `--accept-stale-version` |
...
`human_release`, `catalog_sync`) e `step_intent` ganha campos (`worktree`, `receipt_path`,
`session_ref`). Todo valor novo de `effect_class` nasce com regra de reconciliação declarada.
...
`runtime_stamp` divergente para uma missão aberta interrompe o trabalho e exige uma flag explícita do
operador — no dogfood, isso acontece com frequência, e é o custo aceito. A alternativa (reconciliar de
...
`unit_state`, `attempt.class`, `batch_state`; `effect_class` aditivo), `docs/specs/`
(`stale_workflow_version`, `--accept-stale-version` registrado como `decision`, `ade doctor` reportando
o stamp corrente e o de cada missão aberta), `docs/roadmap.md` (o teste de paridade byte a byte do JCS
