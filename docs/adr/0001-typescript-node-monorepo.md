# ADR 0001 — TypeScript sobre Node 22+, ESM estrito, npm workspaces com um pacote
...
Node ≥ 22 (24.16 instalado), TypeScript estrito, ESM. npm workspaces com **um** pacote na v1,
`packages/core` (engine, adapters, intent, skills, fqe, cli); `packages/web` só na v0.4. Dependências de
runtime da v1: `canonicalize` (JCS), `ajv`, `playwright` (só no FQE). CLI com `node:util` `parseArgs`.
Testes com Vitest, paralelismo por caso com tmpdir próprio por worker. `better-sqlite3` entra junto com
o painel, nunca antes. Fastify, WebSocket, `node-pty` e xterm ficam fora até v0.4/v0.5. pnpm não é
adicionado.
...
conhecido no Windows — resolvido adiando `better-sqlite3`. Um pacote só é cerimônia quase nula, mas
deixa o split preparado quando o painel chegar.
...
dogfood. Custo: baixo — mover arquivos entre workspaces e ajustar `exports`; nenhum contrato publicado
depende do layout de pacotes.

## Consequências para outros documentos

`docs/roadmap.md` (o que cada slice pode importar), `docs/specs/` (engine e CLI), ADR 0013 (painel
carrega `better-sqlite3` e o segundo pacote), ADR 0022 (shims, `lpCommandLine`, caminhos curtos),
`ade doctor` (checagem de versão de Node).
