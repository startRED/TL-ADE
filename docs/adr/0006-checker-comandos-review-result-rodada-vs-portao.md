...
| :--- | :--- | :--- | :--- |
| Checker de **rodada** | após `eval_run:green`, gera rework | `codex exec --json --sandbox read-only --ignore-user-config --output-schema review-result.schema.json` | precisão |
...
`required_action`, mais `deferred` e `rejected`), com `summary` derivado. `no_checker_family_available`
→ `parked`. `claude ultrareview` é portão opcional pré-merge. `codex review` nunca é usado.
...
de 20 stories só no piso do Codex, J3 §6) — pago conscientemente, podado por `--ignore-user-config` e
por `max_diff_bytes` revisto (ADR 0011). `--ignore-user-config` descarta AGENTS.md e skills globais do
usuário: todo contexto do Checker tem de vir pelo Context Pack. `--permission-mode plan` limita o que o
