  ...
  return (
    <>
      <div className={`turn-head${need ? ' hot' : ''}`}>
        ...
        <p>{need ? why : paused ? (m.reason === 'budget' ? 'A missão chegou ao teto de gasto definido em Opções e pausou antes de gastar mais. O que já foi feito está gravado. Aumente o teto em Opções e continue, ou pare por aqui.' : m.reason === 'epic_incomplete' ? REASON.epic_incomplete : m.reason === 'quota' ? `A cota do Gemini acabou. Nada foi gasto nem pulado: a missão retoma sozinha${m.quota_until ? ` às ${new Date(m.quota_until).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : ''}. Para não esperar, troque quem escreve em Modelos e clique em Continuar.` : m.reason === 'plan_failed' ? 'O planejador não entregou o plano de um épico em duas tentativas. Nada foi pulado: ao continuar, ele tenta de novo. Se repetir, troque o modelo do plano intermediário em Modelos.' : 'A IA foi interrompida. A parte em andamento voltou para a fila e o que ela tinha mexido foi desfeito.') : 'Nada para decidir agora.'}</p>
      </div>
