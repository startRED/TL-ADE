...
async function planCritic(plan) {
  ...
  const prompt = [
    ...
    'Leia cada story como se fosse implementá-la agora. Aponte SÓ bloqueios concretos: requisito contraditório, referência obrigatória inexistente (confira), contrato público incompatível, risco sem proteção, dependência entre stories não declarada, duas stories mexendo no mesmo trecho, story que muda um formato ou contrato já coberto por provas de outra story enquanto proíbe tocar nessas provas (contrato impossível), separação sem dependência, risco ou entrega independente que a justifique, critério que manda o sistema seguir sozinho depois de mudar plano, contrato, orçamento ou estado enquanto outro critério ou decisão exige aprovação ou bloqueio para essa mesma mudança (prova impossível), story que cria ou aperta um portão por onde fluxos existentes passam sem listar em scope_paths as provas desses fluxos, story que precisa mudar arquivo que não está no seu scope_paths (confira onde moram as funções que ela altera).',
...
function planPrompt(revising = false) {
  ...
  return [
    ...
    m.epic && m.program?.epics?.some((e) => e.state === 'queued' && e.id !== m.epic.id) ? `Épicos que vêm DEPOIS (não faça o trabalho deles; o que é deles fica para eles): ${m.program.epics.filter((e) => e.state === 'queued' && e.id !== m.epic.id).map((e) => `${e.title}: ${(e.goal || '').slice(0, 200)}`).join(' | ')}` : '',
    attachBlock(m.attachments),
    ...
    // Três regras da parte que mais rodou na TL-ADE (m-mu8usf5z, v03-s6: mais de 14 rodadas; a v03-s5, com os mesmos 6
    // critérios, fechou em 4, então o problema não era o número de critérios): um critério mandava seguir sozinho depois de
    // replanejar enquanto outro e uma decisão exigiam aprovação para a mudança; o portão novo quebrou provas antigas que a story
    // não sabia que podia ajustar; e os arquivos que ela teve de mudar (plan-lifecycle.js, budget.js) não estavam no escopo.
    '- AUTOMÁTICO CONTRA PORTÃO: para cada critério em que o sistema segue sozinho depois de mudar plano, contrato, orçamento ou estado, confira se outro critério ou decisão exige aprovação, validação ou bloqueio para essa MESMA mudança. Se exigir, decida nas decisions qual vence (ex.: "replanejar gera plano novo que para aguardando aprovação") e escreva os dois critérios de acordo. Critérios que se contradizem viram prova impossível e a story roda até o teto.',
    '- PORTÃO NOVO SOBRE FLUXO ANTIGO: story que cria ou aperta um portão (aprovação, validação, orçamento, permissão) por onde fluxos existentes passam põe em scope_paths as provas desses fluxos e ganha um critério "os fluxos existentes X e Y continuam passando com o portão novo (fixtures ajustadas)". Sem isso as provas antigas quebram e quem implementa não sabe se pode ajustá-las.',
    '- ESCOPO COMPLETO: se a story muda uma função que mora em arquivo de outra story ou de parte já commitada, esse arquivo entra em scope_paths. Arquivo necessário fora do escopo vira achado de "fora do contrato" em toda revisão.',
...
const EPICS_JSON_SCHEMA = {
  ...
  properties: {
    ...
    epics: { type: 'array', minItems: 1, maxItems: 10, items: { type: 'object', additionalProperties: false, properties: { id: { type: 'string' }, title: { type: 'string' }, goal: { type: 'string' }, acceptance: { type: 'array', items: { type: 'string' } }, depends_on: { type: 'array', items: { type: 'string' } } }, required: ['id', 'title', 'goal', 'acceptance', 'depends_on'] } },
  },
...
const IMPL_RULES = [
    ...
    // Maior desperdício medido em 20/09: a chamada inteira gasta esperando prova em segundo plano ou subagente, e termina com
    // 0 arquivo alterado. m-mu8usf5z: 174k, 193k e 140k tokens de entrada queimados assim em três chamadas, mais a do épico 9 s3.
    'NUNCA ESPERE: não rode prova em segundo plano, não delegue a subagente e não fique aguardando resultado de nada. A chamada tem teto de turnos, e esperar consome o teto inteiro sem escrever uma linha — chamadas de 140 a 250 mil tokens de entrada já terminaram com 0 arquivo alterado dizendo "aguardando o teste em segundo plano" e "subagent task running, waiting for results". Rode em primeiro plano, com recorte e saída limitada, e siga escrevendo.',
    'Ao rodar provas, rode só o arquivo que importa e LIMITE a saída (reporter compacto, `| tail -40`): a saída inteira entra no seu contexto e gasta cota. Enquanto itera, rode só as provas novas filtrando pelo nome (node --test --test-name-pattern, vitest/jest -t, pytest -k, go test -run); o arquivo inteiro uma vez, no fim: o motor roda tudo de novo depois.',
]
...
const CLOSING = [
  'ANTES DE TERMINAR, confira você mesmo, nesta ordem: (1) cada critério de aceite vale no caminho REAL (o que o CLI ou o usuário executa), não só na prova — fazer a prova passar ajustando o dublê e deixando o caminho real errado é o erro mais comum aqui. Isso se confere com um comando, não com opinião: procure pelo nome de CADA função nova fora do arquivo dela e do arquivo de prova (`grep -rn "nomeDaFuncao" src` ou equivalente). Se só a prova a chama, ela não está integrada e o critério NÃO está atendido; ligue ao caminho real ou declare NÃO FEITO; (2) `git diff --name-only` mostra só arquivo que o CONTRATO permite; (3) você não engoliu erro novo (catch vazio, valor padrão calado).',
...
async function runStory(st, round = 1, previousReview = null, previousVisual = null) {
  ...
  if (round === 1 && st.no_test_phase) { // correção de provas vermelhas: as provas já existem; direto para a implementação
    ...
    // sem foto visual de antes (correção de suíte, ou criada antes desta regra), todo achado antigo contava como novo: a
    // V05-04f (m-mu8usf5z) listou em toda rodada 10 achados de docs/prototypes e proto/src que ela nunca tocou
    if (!st.visual_before && m.plan.needs_ui && state.settings.visual_gate) st.visual_before = await visualGate()
  ...
  if (!st.tests_after.ok) {
    ...
    // trabalho é descartado, as partes dependentes caem junto e o épico volta ao planejador (m-mu8usf5z s7 leu a lista fechada
