...
function planPrompt(revising = false) {
  ...
  return [
    ...
    '- stories: 1 a 4 partes, em ordem de execução. TAMANHO: cada parte = uma fatia que se prova junta (o código, a prova dela e a documentação que ela muda), request com no máximo 160 palavras, acceptance com 2 a 5 critérios, diff esperado de até ~400 linhas, provável de passar numa revisão rigorosa em 1 ou 2 rodadas. Cada parte paga um custo fixo de minutos (prova vermelha, suíte, revisão, commit): NÃO crie parte só de documentação, só de esqueleto ou só de ajuste de texto; isso vai junto da parte de código que o assunto toca. Épico só de documentação vira 1 parte. Número de partes escrito no goal do épico não vale; vale esta regra. Se o trabalho não cabe em 4 partes desse tamanho, faça só a primeira fatia coerente e diga em summary o que ficou para o próximo épico. Cada uma: id (s1, s2…), title, request (instrução completa e autossuficiente para a IA que vai implementar, incluindo o estilo visual quando houver interface), acceptance, test_hint (como provar), depends_on (ids das stories anteriores cujo arquivo, função ou dado esta parte USA; ordem de leitura ou de assunto não é dependência; [] se independente).',
...
const EPICS_JSON_SCHEMA = {
  ...
  properties: {
    ...
    epics: { type: 'array', minItems: 2, maxItems: 6, items: { type: 'object', additionalProperties: false, properties: { id: { type: 'string' }, title: { type: 'string' }, goal: { type: 'string' }, acceptance: { type: 'array', items: { type: 'string' } }, depends_on: { type: 'array', items: { type: 'string' } } }, required: ['id', 'title', 'goal', 'acceptance', 'depends_on'] } },
  },
...
function epicsPrompt() {
  const p = state.project, m = state.mission
  return [
    'Você é o Intent Compiler da TL-ADE. O pedido é grande (vários subsistemas). NÃO escreva stories: divida em ÉPICOS, cada um um subsistema ou fatia coerente que cabe em 1 a 4 partes (cada parte até ~400 linhas de diff, uma prova). Ordem de execução com dependências explícitas; a base vem antes do que depende dela. MENOS ÉPICOS É MAIS RÁPIDO: cada épico paga planejamento e crítica (~15 min) antes de escrever código. Documentação, ADR e emenda de charter vão DENTRO do épico do código que elas autorizam ou descrevem; épico só de papelada só quando a versão inteira é só documentação. O goal diz o que construir, nunca em quantas partes.',
    `Pedido: ${m.request}`,
    attachBlock(m.attachments),
    briefBlock(m.brief),
    `Entendimento prévio: ${m.intent?.summary || ''} Domínios: ${(m.intent?.domains || []).join(', ')}.`,
    ...
    'Responda em português no JSON exigido: title (≤8 palavras); explanation (4 a 8 linhas leigas: o que existirá no fim e o que cada épico entrega); epics (2 a 6; prefira o menor número que mantém cada épico coerente): id (e1, e2…), title (≤8 palavras), goal (instrução completa para planejar esse épico sozinho depois: o que construir, onde, quais arquivos/ADRs ler, o que NÃO fazer), acceptance (2 a 4 critérios observáveis do épico), depends_on (ids anteriores).',
  ].filter(Boolean).join('\n') + skillsBlock(m.skills.planner || [])
}
