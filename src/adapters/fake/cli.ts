 ...
 *
 * @param acao Ação a ser validada.
 * @param scenarioFile Caminho do arquivo para mensagem de erro.
 */
function validateAction(acao: unknown, scenarioFile: string): void {
  ...
  const rec = acao as Record<string, unknown>
 ...
 *
 * @param file Caminho do arquivo de destino.
 * @param text Conteúdo a ser gravado.
 */
 ...
 *
 * @param scenarioDir Diretório absoluto do cenário.
 * @param role Papel do agente.
 * @returns O contador lido (base 0).
 */
export function readCounter(scenarioDir: string, role: string): number {
 ...
 * @param scenarioDir Diretório absoluto do cenário.
 * @param role Papel do agente.
 * @returns Índice consumido antes do incremento.
 */
export function bumpCounter(scenarioDir: string, role: string): number {
 ...
 *
 * @param argv Argumentos de linha de comando (sem executável nem script).
 * @param env Variáveis de ambiente fornecidas.
 * @param [deps] Dependências opcionais injetáveis.
 * @returns Código de saída do processo.
 */
export async function runFakeCli(argv: string[], env: Record<string, string | undefined>, deps: RunFakeCliDeps = {}): Promise<number> {
  ...
  const acao: FakeAction = (acoes[Math.min(currentCount, acoes.length - 1)] ?? {})
  ...
  if (acao.no_result !== true) {
    const resultFile = env.ADE_FAKE_RESULT_FILE as string
    ...
    const rev = (resultData as Record<string, any>)?.input_revision
