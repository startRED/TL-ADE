 ...
 */
export function parseAgyOutput(stdout: string): { envelope: Record<string, unknown> | null; error: null | 'empty' | 'not_json' | 'truncated_json' } {
 ...
 */
export function parseAgyFinding(envelope: Record<string, unknown> | null): { finding: any | null; valid: boolean; errors: string[] } {
  ...
  const findingData: Record<string, any> = structured || envelope
  const claims = Array.isArray(findingData.claims) ? findingData.claims : []
  const hasClaim = claims.some((claim: any) =>
    typeof claim === 'string'
      ? claim.trim() !== ''
 ...
 * @param [role='research']
 */
export function parseAgyUsage(envelope: Record<string, unknown> | null, role: string = 'research'): { cost_usd: number | null; cost_source: 'reported' | 'unknown'; cost_basis: string | null; models: Array<{ role: string; model_id: string }>; tokens: any } {
  const usage = envelope?.usage as Record<string, any> | undefined
