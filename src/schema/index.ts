...
const validators: Map<string,import('ajv').ValidateFunction> = new Map()
...
function loadValidator(schemaName: string) {
...
const legacyValidators: Map<string,import('ajv').ValidateFunction> = new Map()

function loadLegacyValidator(schemaName: string, version: number = 1) {
  ...
  const legacySchemas = (IN_MEMORY_LEGACY_SCHEMAS as Record<string, any>)
  let schema
...
function errorPath(error: import('ajv').ErrorObject) {
...
export function validate(schemaName: string, doc: unknown): { valid: true; errors: [] }|{ valid: false; errors: Array<{ path: string; message: string; code?: string }>; code: 4 } {
  ...
  if (schemaName === 'task-contract' && typeof doc === 'object' && doc !== null) {
    const contract = (doc as Record<string, any>)
    ...
    if (contract.risk && typeof contract.risk === 'object') {
      ...
      const hasSensitive = surfaces.some((s: string) => SENSITIVE_SURFACES.has(s))
      if (hasSensitive) {
        ...
        const validEvidences = evidence.filter(
          (ev: any) => typeof ev === 'string' && TRACEABLE_REF_REGEX.test(ev.trim()),
        )
 ...
 *
 */
export function validateSupported(schemaName: string, doc: unknown, options: { mode?: 'read'|'approval'; forApproval?: boolean } = {}): { valid: true; errors: []; formatVersion: number; current: boolean }|{ valid: false; errors: Array<{ path: string; message: string; code?: string }>; code: 4; formatVersion: number|null; current: false } {
  const version =
    typeof doc === 'object' && doc !== null && typeof (doc as any).format_version === 'number'
      ? (doc as any).format_version
      : null
