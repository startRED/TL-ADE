 ...
 *
 * @param {Array<{ model_id: string }>} reported
 * @param {string | null | undefined} requested
 * @returns {Array<{ role: 'executor' | 'advisor', model_id: string }>}
 */
export function modelsFromUsage(reported, requested) {
  ...
  const models = ids.map((id) => ({ role: /** @type {'executor' | 'advisor'} */ (id === executor ? 'executor' : 'advisor'), model_id: id }))
 ...
 */
export function buildModelTelemetry(input) {
  ...
  const packSections = pack.sections.map((/** @type {any} */ s) => {
  ...
  const sum = packSections.reduce((/** @type {number} */ acc, /** @type {any} */ s) => acc + s.bytes, 0)
  if (sum !== pack.bytes) throw new TelemetryInvalidError(`soma das seções (${sum}) difere de pack_bytes (${pack.bytes})`)

  const skills = (input.skills ?? []).map((/** @type {any} */ s) => {
  ...
  const count = (/** @type {unknown} */ v) => (reportedTokens && isCount(v) ? /** @type {number} */ (v) : 0)
 ...
 * @returns {boolean}
 */
export function isModelCallTelemetry(event) {
 ...
 */
export function telemetryTokens(data) {
 ...
 *
 * @param {{ mission: { id: string, context?: any }, events: any[], outcome: string, capabilitiesDigest: string }} input
 * @returns {{ kind: 'telemetry', data: Record<string, any> } | null}
 */
export function closeMissionSummary({ mission, events, outcome, capabilitiesDigest }) {
