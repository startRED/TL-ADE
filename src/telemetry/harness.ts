import { isModelCallTelemetry } from './telemetry.ts'

const PRUNE_WINDOW = 20
const PRUNE_MIN_RATE = 0.2
const PAIRS_REQUIRED = 5

function optIn(config: any): string[] {
 ...
 *
 */
function cacheByRole(calls: any[]) {
  const byRole: Map<string,{ role: string; tokens_in: number; cache_read: number; ratio: number|null }> = new Map()
 ...
 *
 */
export function auditHarness(events: any[], _config: any) {
  const calls = events.filter(isModelCallTelemetry)
  const seqs = (pred: (e: any) => boolean) => events.filter(pred).map((e) => e.seq)
  const specs: Array<{ id: string; category: string; action: string; bad?: (e: any) => boolean; good?: (e: any) => boolean }> = [
    { id: 'models_identified', category: 'tool_coverage', action: 'registrar o model_id reportado por toda família',
      bad: (e) => isModelCallTelemetry(e) && e.data.models.some((m: any) => m.model_id === 'unknown') },
    ...
    { id: 'pack_sources_cited', category: 'context_efficiency', action: 'exigir fontes citadas no resultado da unidade',
      bad: (e) => isModelCallTelemetry(e) && !e.data.pack_sections.some((s: any) => s.cited) &&
        !e.data.skills_injected.some((s: any) => s.cited) },
  ...
  const checks = specs.map((s) => {
    ...
    const evidence = seqs((s.good as (e: any) => boolean))
    return { id: s.id, category: s.category, pass: evidence.length > 0, evidence, action: s.action }
  })

  const empty = events.length === 0
  const categories: Record<string,{ score: number|null; passed: number; total: number }> = {}
 ...
 *
 */
export function evaluateDefaultPruning({ events, config }: { events: any[]; config: any }) {
  const usage: Map<string,Map<string,boolean>> = new Map()
  const mark = (component: string, story: string, cited: boolean) => {
 ...
 *
 */
export function evaluateModelPromotion({ events, candidate, components, config }: { events: any[]; candidate: string; components: string[]; config: any }) {
  const missing = []
...
/** p90 por posição mais próxima: o menor valor que cobre 90% das observações. @param values */
 ...
 *
 */
export function limitsInForce(config: any): { max_pack_bytes: number; review_max_diff_bytes: number; visual_cut: number } {
  return {
 ...
 *
 */
export function calibrate(events: any[]): { max_pack_bytes?: number; review_max_diff_bytes?: number; visual_cut?: number; motivo: string; amostra: number }|{ proposta: null; motivo: string } {
  const calls = events.filter(isModelCallTelemetry)
  const proposal: { max_pack_bytes?: number; review_max_diff_bytes?: number; visual_cut?: number } = {}
  const reasons = []
  const stories = new Set()

  const ceilings: Array<['max_pack_bytes'|'review_max_diff_bytes',string,any[],number[]]> = [
    ['max_pack_bytes', 'pack_bytes', calls, calls.map((c) => c.data.pack_bytes)],
  ]
  const diffCalls = calls.filter((c) => c.data.pack_sections.some((s: any) => s.section === 'diff'))
  ceilings.push(['review_max_diff_bytes', 'bytes da seção diff', diffCalls,
    diffCalls.map((c) => c.data.pack_sections.filter((s: any) => s.section === 'diff')
      .reduce((acc: number, s: any) => acc + s.bytes, 0))])
 ...
 *
 */
export function pendingCalibration(events: any[], inForce: ReturnType<typeof limitsInForce>): string[] {
  const last = [...events].reverse().find((e) => e?.kind === 'calibration_proposed')
  if (!last) return []
  return (['max_pack_bytes', 'review_max_diff_bytes', 'visual_cut'] as const)
