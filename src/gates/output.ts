 ...
 */
export const EXTRACT_CAPS: Readonly<Record<string, number>> = Object.freeze({
  test: 8192,
  lint: 8192,
  typecheck: 8192,
  build: 8192,
 ...
 */
export const MAX_RAW_REF_BYTES: number = 1024
 ...
 */
function assertRealDir(dir: string): boolean {
  let st
  try {
    st = fs.lstatSync(dir)
  } catch (err) {
 ...
 */
function mkdirContained(dir: string): void {
 ...
 */
function assertPhysicallyContained(missionDir: string, segments: string[], dir: string): void {
 ...
 */
function discardEscapedFile(rawPath: string): void {
  try {
...
interface WriteRawArtifactOptions {
  missionDir: string
  ref: string
  text: string
}

interface WriteRawArtifactResult {
  rawPath: string
  bytes: number
}
 ...
 */
export function writeRawArtifact({ missionDir, ref, text }: WriteRawArtifactOptions): WriteRawArtifactResult {
  ...
  try {
    ...
    if (err && (err as NodeJS.ErrnoException).code === 'ELOOP') {
 ...
 */
function truncateUtf8Buffer(buf: Buffer, maxBytes: number): Buffer {
  if (maxBytes <= 0) {
 ...
 */
function takeLastBytesUtf8(buf: Buffer, maxBytes: number): string {
  if (buf.byteLength <= maxBytes) {
...
export type ExtractStatus = 'success' | 'warning' | 'error'

interface BuildExtractOptions {
  kind: string
...
export interface BuildExtractResult {
 ...
 */
export function buildExtract({ kind, exitCode, expectExit, stdout, stderr, rawRef, bytesRaw }: BuildExtractOptions): BuildExtractResult {
  ...
  if (status !== 'error') {
    ...
    if (fullBytes <= cap) {
      ...
      let prefixBuf: Buffer<ArrayBufferLike> = Buffer.alloc(0)
      let marker = ''
