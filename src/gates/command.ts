 ...
 */
export function resolveGateArgv({ gate, packageJson, vars }: { gate: GateSpec; packageJson: PackageJsonSpec; vars: GateVars }): string[] {
 ...
 */
export async function runContained({ argv, cwd, timeoutS, env }: { argv: string[]; cwd: string; timeoutS: number; env?: Record<string, string> }): Promise<RunContainedResult> {
  validateCmd0(argv?.[0])
