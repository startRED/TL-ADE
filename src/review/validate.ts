 ...
 */
function isPathInsecure(ref: unknown): boolean {
 ...
 */
export function validateEvidenceResult(schemaName: 'unit-result' | 'review-result', doc: any, context: {
...
} = {}): { valid: true; errors: [] } | { valid: false; errors: Array<{ path: string; code: string; message: string }>; code: 4 } {
  ...
  if (!schemaValidation.valid) {
    ...
    const errors: Array<{ path: string; code: string; message: string }> = schemaValidation.errors.map((err) => {
  ...
  function isRefVerified(ref: string): boolean {
 ...
 */
export function isReviewApproved(doc: any, context: any = {}): { approved: boolean; errors: Array<{ path: string; code: string; message: string }> } {
  if (!doc || typeof doc !== 'object') {
    return {
      approved: false,
      errors: [
