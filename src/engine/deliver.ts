...
export async function deliverStory({
  ...
  }): Promise<{ delivered: boolean; reason: string; commit: string | null }> {
 ...
 */
export function withDeliveryFlag(journal: { append: (event: Record<string, unknown>) => Promise<Record<string, unknown>> }): { append: (event: Record<string, unknown>) => Promise<Record<string, unknown>> } {
  return {
    append: (partial) => {
