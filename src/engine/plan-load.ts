...
import { assertCallBudget } from './budget.ts'
...
export interface LoadedPlan {
  ...
  planDir: string
  missionBudget: { max_usd: number; max_wall_clock_seconds: number; max_parked_units: number; max_subscription_weekly_percent: number} 
  gates: GateSpec[]
  stories: LoadedStory[]
  lineageCallsRemaining?: number
}

/**
 * Retorna o orçamento padrão de modelo e correções por complexidade e presença de UI.
 */
export function defaultStoryBudget({ complexity, needs_ui = false }: { complexity: string; needs_ui?: boolean }): { max_model_calls: number; max_rework_rounds: number } {
 ...
 */
export function loadPlan(planPath: string | Record<string, any>, options: { mode?: 'read' | 'approval' } = {}): LoadedPlan {
  let doc
  let planDir
  ...
  if (doc && typeof doc === 'object') {
    const checkEffectEntry = (k: string, v: unknown) => {
      if (typeof v !== 'boolean') {
  ...
  if (fs.existsSync(gatesPath)) {
    ...
    for (let i = 0; i < parsedGates.length; i++) {
      ...
      if (
        ...
        item.argv.length === 0 ||
        !item.argv.every((a: any) => typeof a === 'string')
      ) {
      ...
      for (const storyId of epic.stories ?? []) {
        ...
        for (let i = 0; i < verifiersList.length; i++) {
          ...
          for (const cand of candidates) {
            ...
            const inScope =
              !isTraversal && scopePaths.some((p: string) => matchesGlob(p, normalized))
            if (!inScope) {
              throw new AdeError(
                'eval_outside_scope',
        ...
        const rawEvals: any[] = Array.isArray(contract.evals) && contract.evals.length > 0
          ? contract.evals
          : (Array.isArray(contract.verifiers) ? contract.verifiers : [])
        const normalizedEvals = rawEvals.map((e: any, idx: number) => {
          const { cmd, ...rest } = e
