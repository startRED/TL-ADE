 ...
 */
export function assertCallBudget(budget: { max_model_calls?: any }, where: 'plan' | 'contract'): void {
 ...
 */
export function observedUsd(events: Array<Record<string, any>>): { observed_usd: number; unobserved_calls: number } {
  let observed_usd = 0
 ...
 */
export function checkUsdCap({ observed_usd, max_usd }: { observed_usd: number; max_usd: number }): {
  ok: boolean
 ...
 */
export function checkMissionBudget({ events = [], budget = {}, now = Date.now(), states = {} }: { events?: Array<Record<string, any>>; budget?: Record<string, any>; now?: number; states?: Record<string, any> | Array<any> } = {}): {
  allowed: boolean
  ...
  if (budget.max_model_calls !== undefined && budget.max_model_calls !== null) {
    ...
    const unitStats: Map<string, { reserved: number; executedStepIds: Set<string>; otherExecuted: number }> = new Map()
    ...
    for (const event of evts) {
      ...
      let stats = unitStats.get(unit)
      if (!stats) {
        stats = { reserved: 0, executedStepIds: new Set(), otherExecuted: 0 }
 ...
 */
function getEventUnit(event: Record<string, any>): string | null {
 ...
 */
export function getOpenReservations(events: Array<Record<string, any>>): Array<Record<string, any>> {
 ...
 */
export function validateQuotaReceipt(receipt: any, { family, max_percent = 50, now = Date.now() }: { family?: string; max_percent?: number; now?: number | string | Date } = {}): {
  ok: boolean
...
export function authorizePaidCall(params: { events?: Array<Record<string, any>>; mission_budget?: Record<string, any>; story_budget?: Record<string, any>; family?: string; phase?: string; requested_usd?: number; requested_calls?: number; requested_turns?: number; used_turns?: number; context_bytes?: number; context_limit?: number; quota_receipt?: any; now?: number | string | Date; states?: Record<string, any> | Array<any>; observed_usd?: number; open_reservations?: Array<any> } = {}): {
  ...
  if (phase) {
    ...
    const turnLimits: Record<string, number> = PHASE_TURN_LIMITS
