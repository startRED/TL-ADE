 ...
 */
export function getStoryDeps(story: any): string[] {
 ...
 */
export function isBlocked(state: any): boolean {
  if (!state) return false
 ...
 *
 * @param preserved Entrada de `decision/stories_preserved`.
 * @param status Status declarado na entrada preservada.
 */
function preservedDivergence(preserved: any, status: string, storiesById: Map<string, any> | null): string | null {
 ...
 *
 * @param [storiesById] Contratos canônicos carregados, indexados por id.
 ...
 */
export function deriveStoryStates(events: Array<Record<string, any>>, storiesById: Map<string, any> | null = null): Record<string, { status: string; commit?: string | null; reason?: string | null }> {
  
  const states: Record<string, { status: string; commit?: string | null; reason?: string | null }> = {}
 ...
 */
export function validateGraph(stories: Array<{ id: string; depends_on?: string[] }>, satisfiedDeps = new Set()): void {
 ...
 */
export function nextReady(stories: Array<{ id: string; depends_on?: string[] }>, states: Record<string, any> = {}): { id: string } | null {
 ...
 */
export function checkApproval(deps: any, loadedPlan: import('./plan-load.ts').LoadedPlan, events: Array<Record<string, any>>, mDir: string): { valid: boolean; reason?: string; storyId?: string; nextAction?: string; approval?: any } {
 ...
 * aprovação congelada, retomada após interrupção e replanejamento com no_changes.
 */
export async function runSequentialMission(deps: Record<string, any>, { loaded, repoDir, missionDir }: { loaded: import('./plan-load.ts').LoadedPlan; repoDir: string; missionDir: string }): Promise<{
  ...
  const readEvents = (mDir: string): Array<Record<string, any>> => {
   ...
   */
  const controlGate = async (completed: string[]) => {
    ...
    if (ctl.state === 'STOPPED') {
      return { status: 'stopped' as const, exitCode: 3, completedStories: completed, reason: 'mission_stopped', nextAction }
    }
    const { shutdown, journal } = (deps)
   ...
   */
  const handled: Map<string, { status: string; commit: string | null; reason: string | null }> = new Map()
  ...
  try {
    ...
    // Executa a story
    const runStoryFn = deps.runStory || (await import('../engine.ts')).runStory
    ...
    if (storyResult.reason === 'no_changes') {
      ...
      if (untouchedPending.length > 0) {
        ...
        const allPendingIds = [
          ...
          ...replanRes.replannedStories.map((c: any) => c.id),
        ]
      ...
      const widened = [
        ...(newAuth.permitted_effects || []).filter((e: string) => !(frozenAuth.permitted_effects || []).includes(e)),
        ...(newAuth.eligible_skills || []).filter((s: string) => !(frozenAuth.eligible_skills || []).includes(s)),
      ]
      if (widened.length > 0) {
        return {
          status: 'awaiting_operator',
