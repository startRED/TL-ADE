 ...
 *
 * @returns A classe de Database do better-sqlite3
 */
export function checkNativeSqlite(): any {
  try {
 ...
 */
export async function rebuildProjection({ repoDir = process.cwd(), indexPath }: { repoDir?: string; indexPath?: string } = {}): Promise<{ missions: number; events: number; digest: string }> {
