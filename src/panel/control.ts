...
import { activeTakeover, readMissionControl, requestMissionControl } from '../engine/control.ts'
...
import { createTakeoverSession, loadPtyAdapter } from './pty.ts'
...
let ownStartTime: Promise<string | null> | null = null
...
export function createInterventionController({ repoDir, terminal = {} }: { repoDir: string; terminal?: TerminalDeps }) {
  
  const sessions: Map<string, { session: ReturnType<typeof createTakeoverSession>; journal: import('../journal/journal.ts').Journal; lease: { release: () => Promise<void> }; socket: import('node:net').Socket | null }> = new Map()
  ...
  function loadMission(body: any) {
    ...
    return { missionId, storyId, missionDir, digest: (body.digest as string), events }
  }

  const describe = ({ session }: { session: ReturnType<typeof createTakeoverSession> }) => ({
  ...
  const takeover = (body: any) => serial(async () => {
  ...
  const release = (body: any) => serial(async () => {
   ...
   */
  function findTerminal({ token, mission, story }: { token: string | null; mission: string | null; story: string | null }) {
