 ...
 */
export async function loadPtyAdapter(): Promise<{ spawn: Function }> {
...
export function createTerminalOutputFilter(): (text: string) => string {
  ...
  let state: 'text' | 'esc' | 'string' | 'string_esc' = 'text'
  return (text) => {
    let out = ''
...
export function createTakeoverSession({
  ...
  const listeners: Set<(chunk: Buffer) => void> = new Set()
  let exited = false
  ...
  return {
    ...
    sessionRef,
    worktree,
    pid: (pty.pid as number),
    
    onOutput(cb: (chunk: Buffer) => void) {
      listeners.add(cb)
