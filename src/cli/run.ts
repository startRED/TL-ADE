...
function createHardPreconditionPorts({ repoDir, missionDir, missionId, loaded, events, gitPort }: {
        ...
        gitPort: any
    }): Record<string, { check: () => Promise<import('../engine/preflight.ts').PreflightCheckResult> }> {
  ...
  return {
    ...
    permitted_effects: {
      check: async () => {
        ...
        const widened = planned.filter((e: string) => !approved.includes(e))
...
export function heartbeatAgeMs(missionDir: string): number | null {
  try {
...
export async function runCommand(options: {
  ...
  let journal: import('../journal/journal.ts').Journal | null = null
  
  let lease: import('../lease/lease.ts').Lease | null = null
  ...
  let missionExitCode: number | null = null
...
`)
  ...
  try {
    ...
    const engineDeps = {
      ...
      preflight: async ( { story, loaded, repoDir, events }: { story: any; loaded: any; repoDir: string; events: any[] }) => {
    ...
    if (journal && missionStarted) {
      ...
      const summary = closeMissionSummary({
        mission: { id: missionId, context: (loaded.plan as any).context },
