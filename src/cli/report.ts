...
import { telemetryTokens } from '../telemetry/telemetry.ts'
import { calibrate, limitsInForce, pendingCalibration } from '../telemetry/harness.ts'
 ...
 */
function readApprovedConfig(missionDir: string) {
 ...
 */
async function recordCalibration(missionDir: string, events: any[], config: any, stdout: { write: (s: string) => void }) {
 ...
 */
export function sumTokensByRole(events: Array<Record<string, any>> = []): Array<{ role: string; calls: number; unavailable_calls: number; input: number; cache_write: number; cache_read: number; output: number; usd: number }> {
  
  const grouped: Record<string, { role: string; calls: number; unavailable_calls: number; input: number; cache_write: number; cache_read: number; output: number; usd: number }> = {}
 ...
 */
export function sumQuotaUsage(events: Array<Record<string, any>> = [], nowMs: number | string | Date | (() => number) = Date.now()): {
  ...
  const dailyGroups: Map<string, { day: string; family: string; role: string; quota_tokens: number; unavailable_calls: number }> = new Map()

  
  const windowGroups: Map<string, { family: string; role: string; last_5h: number; last_7d: number }> = new Map()

  
  const latestReceipts: Map<string, { family: string; used_percent: number; reserved_percent: number; observed_at: string; weekly_reset_at: string }> = new Map()
 ...
 */
export function renderReport(mission: string, units: Array<{ unit: string; status: string; reason?: string | null; commit?: string | null }>, costs: Array<{ role: string; calls: number; unavailable_calls: number; input: number; cache_write: number; cache_read: number; output: number; usd: number }> = [], quota: {
    daily?: Array<{ day: string; family: string; role: string; quota_tokens: number; unavailable_calls: number }>
    windows?: Array<{ family: string; role: string; last_5h: number; last_7d: number }>
    receipts?: Array<{ family: string; used_percent: number; reserved_percent: number; observed_at: string; weekly_reset_at: string }>
 ...
 */
export function renderParkedUnits(events: Array<Record<string, any>> = []): string {
 ...
 */
export function renderVisualComparison(events: Array<Record<string, any>> = []): string {
  ...
  const r1Caps: Array<Record<string, any>> = r1Eval?.captures || []
  
  const r2Caps: Array<Record<string, any>> = r2Eval?.captures || []
