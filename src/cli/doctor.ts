 ...
 */
async function defaultGitConfigImpl(): Promise<string | null> {
  try {
...
type ResolvedClaude = { exe: string; prefixArgs: string[]} 
 ...
 */
type ResolveImpl = (cmd: string) => ResolvedClaude | Promise<ResolvedClaude>
 ...
 */
async function defaultProbeImpl(resolved: { exe: string; prefixArgs: string[] }, args: string[]): Promise<{ stdout: string; exitCode: number | null }> {
  try {
    ...
    const code = (err as { code?: unknown, stdout?: unknown, message: string }).code
    if (typeof code === 'number') {
      return { stdout: String((err as { stdout?: unknown }).stdout ?? ''), exitCode: code }
    }
    throw new AdeError('doctor_probe_failed', (err as Error).message, 1)
  }
}
 ...
 */
async function defaultHelpImpl(resolved: { exe: string; prefixArgs: string[] }): Promise<{ exitCode: number | null }> {
  try {
 ...
 */
function readFixtureDoc(fixturePath: string | URL): Record<string, unknown> {
  try {
 ...
 */
function validateCapabilitySet(doc: Record<string, unknown>): void {
 ...
 */
function writeCapabilitiesFile(homeDir: string, doc: Record<string, unknown>): string {
 ...
 */
async function computeLongpathsWarnings(platform: string, gitConfigImpl: () => Promise<string | null>): Promise<{ longpaths: string | null; warnings: string[] }> {
  let longpaths
  try {
...
export async function runDoctor(opts: {
  ...
  if (offline) {
    ...
    let resolved: { exe: string; prefixArgs: string[] }
    try {
    ...
    if (probeFailed) {
      ...
      const usage = (
        envelope?.usage as { input_tokens?: unknown, cache_creation_input_tokens?: unknown } | undefined)
      const inputTokens = Number.isInteger(usage?.input_tokens) ? (usage?.input_tokens as number) : 0
      ...
      const modelUsage =
        (envelope?.modelUsage as Record<string, { contextWindow?: unknown }> | undefined) ?? {}
 ...
 */
export function diagnoseSkills({ homeDir = os.homedir(), repoDir = process.cwd(), catalogDir = path.join(homeDir, '.ade', 'catalog') }: { homeDir?: string; repoDir?: string; catalogDir?: string } = {}): { deltas: any[]; oversizedDocs: any[]; quarantineCount: number; currentHashes: Record<string, string> } {
  const monitoredDirs = [
 ...
 */
export function diagnoseHarness(repoDir: string) {
...
export async function main(argv: string[], deps: Record<string, any> = {}) {
  ...
  if (argv.includes('--docs')) {
    ...
    let diagnosis: ReturnType<typeof diagnoseDocs>
    try {
