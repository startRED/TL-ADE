...
/**
 * @typedef {'persuade' | 'operate' | 'read' | 'experience'} SurfaceMode
 */

/**
 * @typedef {Object} DesignBrief
 * @property {string} audience
 * @property {SurfaceMode} surface_mode
 * @property {string[]} preserved_patterns
 * @property {string[]} avoid_patterns
 * @property {string[]} references
 * @property {{ name: string, signature: string, self_critique: string }} direction
 * @property {{ variance: number, motion: number, density: number, justification: string }} character
 * @property {{ palette: Record<string, string>, typography: { display: string, body: string }, spacing: string[], radii: string[] }} tokens
 */
 ...
 * @param {string} [declaredMode]
 * @returns {SurfaceMode}
 */
export function inferSurfaceMode(request, declaredMode) {
  if (declaredMode && ['persuade', 'operate', 'read', 'experience'].includes(declaredMode)) {
    return /** @type {SurfaceMode} */ (declaredMode)
  }
...
export function buildDesignBrief({ request, discovery = {}, repoSignals = {}, surfaceMode }) {
  ...
  const preservedPatterns = Array.from(
    new Set([...(repoSignals.preserved_patterns || []), .../** @type {any[]} */ (discovery.anchors || []).map((a) => a.path)]),
  )
  ...
  const references = [
    ...
    .../** @type {string[]} */ (repoSignals.motion_libs || []).map((m) => `lib:${m}`),
  ]
