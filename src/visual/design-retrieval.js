 ...
 * @returns {number}
 */
function bm25Score(queryTokens, docTokens, avgDocLen, k1 = 1.2, b = 0.75) {
 ...
 *
 * @param {any} [providedCatalog]
 * @returns {any}
 */
export function loadDesignCatalog(providedCatalog) {
...
export function recommendDesign({ brief, catalog, limit = 3 }) {
  ...
  const macrostructures = /** @type {any[]} */ (cat.macrostructures || [])
  let bestMacro = null
  ...
  const components = /** @type {any[]} */ (cat.components || [])
  ...
  // 3. Avalia paleta
  const palettes = /** @type {any[]} */ (cat.palettes || [])
  const bestPalette =
