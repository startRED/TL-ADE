...
export async function runVisualGates(params) {
  ...
  const consoleAllowlist = /** @type {string[]} */ (visualConfig.console_allowlist || []).map((p) => new RegExp(p))
  const networkAllowlist = /** @type {string[]} */ (visualConfig.network_allowlist || []).map((p) => new RegExp(p))
  const activeGates = new Set(visualConfig.gates || ['D1', 'D2', 'D3', 'D4', 'D5', 'D6'])

  /** @type {Record<'D1' | 'D2' | 'D3' | 'D4' | 'D5' | 'D6', GateResult>} */
  const results = {
  ...
  if (activeGates.has('D3') && page) {
    try {
      ...
      const hasAxe = await page.evaluate(() => typeof /** @type {any} */ (window).axe !== 'undefined').catch(() => false)
      if (!hasAxe) {
      ...
      const axeResult = await page.evaluate(async () => {
        if (typeof /** @type {any} */ (window).axe === 'undefined') return { violations: [] }
        return /** @type {any} */ (window).axe.run(document, { runOnly: ['color-contrast'] })
      })
      ...
      if (violations.length > 0) {
        ...
        const totalNodes = violations.reduce((/** @type {number} */ acc, /** @type {any} */ v) => acc + (v.nodes?.length || 0), 0)
  ...
  if (activeGates.has('D4')) {
    const routeConfig = /** @type {any[]} */ (visualConfig.routes || []).find((r) => r.path === route)
  ...
  if (activeGates.has('D5')) {
    if (detector && typeof detector.detect === 'function') {
      try {
        ...
        const blockingIssues = /** @type {any[]} */ (issues).filter((iss) => iss.category === 'slop' || iss.severity === 'error')
