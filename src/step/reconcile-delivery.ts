 ...
 */
function assertNonEmptyString(val: unknown, name: string): void {
...
function isNetworkError(err: unknown): boolean {
  ...
  const anyErr = (err as { code?: unknown, message?: unknown, cause?: unknown })
  const code = anyErr.code
  if (typeof code === 'string' && NETWORK_ERROR_CODES.has(code)) {
    return true
  }
  if (anyErr.cause && typeof anyErr.cause === 'object') {
    const causeCode = (anyErr.cause as { code?: unknown }).code
 ...
 */
function buildNetworkUnavailableResult(err: unknown): {
verdict: 'ambiguous'
reason: 'network'
state: 'awaiting_operator'
handoff: 'awaiting_operator'
 ...
 */
function extractForgeIntentRefs(intent: Record<string,any>): { baseRef: string; headCommit: string|null; headRef: string } {
  const intentContext = intent.intent_context ?? {}
  const baseRef =
    intent.base_ref ??
    intent.base ??
 ...
 */
async function queryForgePullRequest(forgePort: { findPullRequest: (params: { headRef?: string }) => Promise<any> }, headRef: string): Promise<{ ok: true; pr: any }|{ ok: false; result: ReturnType<typeof buildNetworkUnavailableResult> }> {
  try {
