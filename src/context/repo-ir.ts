...
function assertInference(item: any, index: number): { confidence: number; evidence_refs: string[]; provenance: 'inference' } {
  ...
  if (
    ...
    evidence_refs.some((r: any) => typeof r !== 'string' || r.length === 0)
  ) {
...
export async function buildRepoIr({
  ...
  const rawFacts: any[] = discovery.facts || []
  ...
  const rawSymbols: any[] = discovery.symbols || []
