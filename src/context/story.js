...
import { FolderWorkspace } from '../workspace/folder.js'
 ...
 * @returns {boolean}
 */
function isPathInScope(filePath, scopePaths = ['src/**'], doNotTouch = []) {
 ...
 * @returns {string}
 */
function extractRecentOperatorNotes(operatorNotes) {
 ...
 * }>}
 */
export async function buildStoryContext(input, deps = {}) {
 ...
 * @returns {boolean}
 */
function resolvesToExecutable(execName, workspaceDir) {
 ...
 *
 * @param {{ story: any, contract?: any, workspaceDir?: string }} options
 * @returns {{ status: 'ready' } | { status: 'story_pack_overflow', reason: string, error: AdeError } | { status: 'refused', reason: string }}
 */
export function guardStoryContext({ story, contract, workspaceDir }) {
  ...
  if (contractBytes > CONTRACT_MAX_BYTES) {
    return {
      status: /** @type {const} */ ('story_pack_overflow'),
  ...
  for (const cmd of commands) {
    if (!Array.isArray(cmd) || cmd.length === 0 || typeof cmd[0] !== 'string' || !cmd[0].trim()) {
      return { status: /** @type {const} */ ('refused'), reason: 'eval_cmd_unknown' }
    }
    const execName = cmd[0].trim()
    if (FORBIDDEN_EXECUTABLES.has(execName.toLowerCase())) {
      return { status: /** @type {const} */ ('refused'), reason: 'verifier_command_forbidden' }
    }
    if (!resolvesToExecutable(execName, workspaceDir)) {
      return { status: /** @type {const} */ ('refused'), reason: 'verifier_command_missing' }
    }
  }

  return { status: /** @type {const} */ ('ready') }
}
 ...
 * @returns {Promise<any>}
 */
export async function prepareStoryContext(options, deps = {}) {
