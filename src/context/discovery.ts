 ...
 */
async function readText(workspace: import('../workspace/port.ts').WorkspacePort, relPath: string): Promise<string> {
 ...
 */
export async function discoverProject(workspace: import('../workspace/port.ts').WorkspacePort, options: {
  ...
  const rootDir = (workspace as any).worktreeDir || (workspace as any).rootDir || process.cwd()

  // 1. Estado da árvore
  let clean = true
  const gitPort = (workspace as any).gitPort
  ...
  if (snap.paths.includes('package.json')) {
    ...
    try {
      ...
      throw new DiscoveryManifestError('package.json', (err as Error).message)
    }
    {
