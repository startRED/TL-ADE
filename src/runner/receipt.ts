...
export type ReceiptState = 'starting' | 'running' | 'exited' | 'timeout' | 'crashed' | 'start_failed'
...
export type Receipt = {
  ...
  process_fingerprint: ProcessFingerprint | null
  started_at: string
 ...
 *
 */
function hasCanonicalDateComponents(match: RegExpExecArray): boolean {
 ...
 *
 */
export function isValidRequest(request: unknown): request is SpawnRequest {
...
// getProcessStartTime (src/lease/process-info.ts) devolve dois formatos reais, não o ISO de 3
 ...
 *
 */
function isValidFingerprintStartTime(value: unknown): value is string {
 ...
 *
 */
export function isValidFingerprint(fp: unknown): fp is ProcessFingerprint {
 ...
 *
 */
export function receiptPath(missionDir: string, stepId: string): string {
 ...
 *
 */
export function withRunning(receipt: Receipt, fingerprint: unknown, nowIso: string = new Date().toISOString()): Receipt {
 ...
 *
 */
export function isValidReceipt(parsed: unknown): parsed is Receipt {
  ...
  if (
    ...
    !RECEIPT_STATES.includes((r.state as ReceiptState))
  ) {
    return false
  }
 ...
 *
 */
export function writeReceipt(file: string, receipt: Receipt, { fsyncSync = fs.fsyncSync }: { fsyncSync?: (fd: number) => void } = {}): void {
 ...
 *
 */
export function readReceipt(file: string): Receipt|null {
  if (typeof file !== 'string' || !file) {
    throw new TypeError('file inválido')
  }
  let content: string
 ...
 *
 */
export function chargeOf(state: string): 'released'|'charged' {
  if (!RECEIPT_STATES.includes((state as ReceiptState))) {
