 ...
 *   topK?: number,
 * }} options
 * @returns {Array<{ id: string, score: number, entry?: any }>}
 */
export function rankSkills({ story = {}, entries = [], topK = 8 }) {
  ...
  /** @type {Record<string, number>} */
  const avglen = {}
  for (const field of /** @type {Array<keyof typeof weights>} */ (Object.keys(weights))) {
  ...
  const scored = docs.map((doc) => {
    ...
    for (const term of queryTokens) {
      ...
      for (const [field, w] of /** @type {Array<[keyof typeof weights, number]>} */ (Object.entries(weights))) {
