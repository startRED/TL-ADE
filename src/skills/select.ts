import { rankSkills } from './bm25.ts'
...
export function selectStorySkills({ story = {}, candidates = [], selector, budget = {} }: {
    ...
    }): Array<{ id: string; name?: string; score: number; source: string; sha256: string; bodyTokens: number; references: string[];[key: string]: any }> {
  if (!Array.isArray(candidates) || candidates.length === 0) {
    return []
  }

  const storyDomains = (story.domains || (story.domain ? [story.domain] : [])).map((d: any) => String(d).toLowerCase())
  const storyLanguages = (story.languages || (story.language ? [story.language] : [])).map((l: any) =>
  ...
  const filtered = candidates.filter((cand) => {
    ...
    // Domínios
    const candDomains = (cand.domains || (cand.domain ? [cand.domain] : [])).map((d: any) => String(d).toLowerCase())
    if (storyDomains.length > 0 && candDomains.length > 0) {
      const hasDomainMatch = candDomains.some((d: string) => storyDomains.includes(d))
      if (!hasDomainMatch) {
        return false
      }
    }
    ...
    // Linguagens
    const candLanguages = (cand.languages || (cand.language ? [cand.language] : [])).map((l: any) =>
      String(l).toLowerCase(),
    )
    if (storyLanguages.length > 0 && candLanguages.length > 0) {
      const hasLangMatch = candLanguages.some((l: string) => storyLanguages.includes(l))
      if (!hasLangMatch) {
        return false
      }
    }
    ...
    // Famílias
    const candFamilies = (cand.families || []).map((f: any) => String(f).toLowerCase())
