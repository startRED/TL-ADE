...
import { scanSkill } from './skillguard.ts'
 ...
 */
export function isBlockedLicense(lic: string): boolean {
 ...
 */
export function parseSkillMarkdown(content: string): { frontmatter: Record<string, any>; body: string } {
 ...
 */
function isPathDeclared(filePath: string, declaredPaths: string[] = ['skills/**']): boolean {
...
export async function syncCatalog({ config = {}, catalogDir }: {
    ...
    }): Promise<{ indexPath: string; entries: any[]; digest: string }> {
...
export function listCatalog({ index, domain, trust, source }: {
  ...
  return index.entries.filter((e) => {
    if (domain) {
      ...
      const domains = (e.domains || []).map((d: any) => String(d).toLowerCase())
 ...
 */
export function loadApprovedSkills({ catalogDir, approvedSkills }: { catalogDir: string; approvedSkills: string[] }): { skills: any[]; snapshot: Array<{ id: string; sha256: string }> } {
  ...
  for (const id of [...new Set(approvedSkills)].sort()) {
    const entry = index.entries?.find((candidate: any) => candidate.id === id)
