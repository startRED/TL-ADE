...
export function planPrompt(request: string, repoDir: string, understanding: Understanding, answers: string, briefingBlock: string, skills: Array<{ id: string; summary: string }> = []): string {
  return [
    ...
    skills.length > 0
      ? ['- skills: ids desta lista com a melhor combinação PARA AQUELA PARTE, pensada no que ela faz (não repita a mesma combinação em toda parte). Sem número fixo: parte grande ou de várias frentes leva vários guias, parte simples leva um ou nenhum ([]). Cada guia vai para um papel: com review no id vai para o revisor (inclua um de segurança se a parte mexe com entrada externa, credencial ou rede); com test, tdd, testing ou e2e no id vai para quem escreve a prova e para quem escreve o código (escolha o do tipo de teste da parte: unidade, componente de interface ou ponta a ponta); os outros vão para quem escreve o código: o da linguagem e dos tipos quando pesam, o do tipo de trabalho (interface, API, banco, erros, documentação, migração) e, se servir, um de método. Parte de interface leva os guias de qualidade visual que servirem. Nunca dois guias que dizem a mesma coisa. ponytail e ponytail-review entram sozinhos em toda parte de código.', ...skills.map((k) => `  - ${k.id}: ${k.summary}`)].join('\n')
