 ...
 */
export function splitContract(contract: any, limits: { max_scenarios?: number; max_contract_bytes?: number } = {}): any[] {
  ...
  for (const [name, value] of ([['max_scenarios', maxScenarios], ['max_contract_bytes', maxBytes]] as Array<[string, number]>)) {
 ...
 */
function buildParts({ contract, scenarios, verifiers, requirements, chunkSize }: { contract: any; scenarios: any[]; verifiers: any[]; requirements: any[]; chunkSize: number }) {
