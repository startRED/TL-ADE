...
export function digestOf(value: any) {
 ...
 */
export function isSufficient(result: any): boolean {
 ...
 */
export function reversibleDefault(unknown: any): { value: any; why: string } {
  ...
  const recommended = (unknown.options || []).find((o: any) => o && o.recommended)
  if (recommended?.id) {
 ...
 */
export function fallbackArtifact({ unknown, budget = {}, reason, fallbackAuthorized = true }: { unknown: any; budget?: any; reason: string; fallbackAuthorized?: boolean }): any {
 ...
 */
export async function runResearchStep({ unknown, budget = {}, researcher, policy = {} }: { unknown: any; budget?: any; researcher?: Function; policy?: any }): Promise<any> {
  ...
  if (Array.isArray(result?.team_results)) {
    ...
    const team: any[] = result.team_results
  ...
  const claims = Array.isArray(result.claims)
    ? result.claims.map((c: any) => (typeof c === 'string' ? { text: c } : c))
    : []
