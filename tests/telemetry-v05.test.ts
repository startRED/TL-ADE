...
import { parseTokens, parseUsage } from '../src/adapters/claude/parse.ts'
import { parseCodexTokens } from '../src/adapters/codex/parse.ts'
import { dispatchAgy, setAgyAvailable } from '../src/adapters/agy/index.ts'
import { sumQuotaUsage, sumTokensByRole } from '../src/cli/report.ts'
...
describe('v0.5 telemetria completa e auditoria do harness', () => {
  ...
  test('criterio_1_toda_model_call_gera_telemetria_completa_inclusive_pesquisa_e_falha', async () => {
    ...
    // sem US$ do agy, os tokens vezes o preço de lista viram estimativa (25/09)
    expect(res.telemetry).toMatchObject({ cost_source: 'estimated', cost_usd: (10 * 0.75 + 5 * 3.75) / 1_000_000 })
